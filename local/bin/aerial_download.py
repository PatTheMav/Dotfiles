...
time_information = {
    ...
    'F439B0A7-D18C-4B14-9681-6520E6A74FE9': 'night',
    ...
    '78911B7E-3C69-47AD-B635-9C2486F6301D': 'day',
    'D60B4DDA-69EB-4841-9690-E8BAE7BC4F80': 'day',
    'E556BBC5-D0A0-4DB1-AC77-BC76E4A526F4': 'day',
    '7719B48A-2005-4011-9280-2F64EEC6FD91': 'day',
    '63C042F0-90EF-4A95-B7CC-CC9A64BF8421': 'day',
    'BAF76353-3475-4855-B7E1-CE96CC9BC3A7': 'night',
    '30313BC1-BF20-45EB-A7B1-5A6FFDBD2488': 'night',
    '89B1643B-06DD-4DEC-B1B0-774493B0F7B7': 'night',
    'EC67726A-8212-4C5E-83CF-8412932740D2': 'night',
    ...
    'EE01F02D-1413-436C-AB05-410F224A5B7B': 'night',
    'E99FA658-A59A-4A2D-9F3B-58E7BDC71A9A': 'night',
    '3E94AE98-EAF2-4B09-96E3-452F46BC114E': 'night',
    '29BDF297-EB43-403A-8719-A78DA11A2948': 'night',
    '82BD33C9-B6D2-47E7-9C42-AA3B7758921A': 'night',
    '3D729CFC-9000-48D3-A052-C5BD5B7A6842': 'night',
    'F604AF56-EA77-4960-AEF7-82533CC1A8B3': 'night',
    '7F4C26C2-67C2-4C3A-8F07-8A7BF6148C97': 'night',
    'F5804DD6-5963-40DA-9FA0-39C0C6E6DEF9': 'night',
    '640DFB00-FBB9-45DA-9444-9F663859F4BC': 'night',
    '44166C39-8566-4ECA-BD16-43159429B52F': 'night',
    '03EC0F5E-CCA8-4E0A-9FEC-5BD1CE151182': 'night',
    '737E9E24-49BE-4104-9B72-F352DE1AD2BF': 'night',
}
...
def _find_duplicate(video_id, video_url):
    ...
    for video in processed:
        if video.get('video_id', '') == video_id:
            return (True, video)
        elif video_url is not None and video.get('urls', {}).get('h264', None) is not None:
            if os.path.basename(video_url) == os.path.basename(video.get('urls', {}).get('h264')):
                return (True, video)
...
def _parse_aerial_files(json_folder):
    folder_names = [
        'tvOS 13',
        'tvOS 12',
        'tvOS 11',
    ...
    for folder_name in folder_names:
        json_folder_path = os.path.realpath(f'{json_folder}/{folder_name}')
        if os.path.exists(json_folder_path):
            with open(f'{json_folder_path}/{file_name}') as json_file:
                json_data = json.load(json_file)
                aerial_assets = _parse_aerial_json(json_data, folder_name)

                processed.extend(aerial_assets)
        else:
            raise PathError('specified JSON path is not valid')
...
def _start_download(file_list, formats):
    ...
    for (location, scenes) in file_list.items():
        ...
        for num, scene in enumerate(reversed(scenes), start=1):
            prepared_list = {video_format:(url, _build_filename(url, location, num)) for video_format, url in scene.items() if url is not None}

            for requested_format in formats:
                if requested_format == 'best-hdr':
                    url, destination = prepared_list.get(
                        '4k-hdr', False) or prepared_list.get(
                        'hdr', False) or (None, None)
                elif requested_format == 'best-sdr':
                    url, destination = prepared_list.get(
                        '4k-hevc', False) or prepared_list.get(
                        'hevc', False) or prepared_list.get(
                        'h264', False) or (None, None)
                else:
                    url, destination = prepared_list.get(requested_format, (None, None))

                if url and destination:
                    response = requests.get(url, stream=True, verify=('AppleIncRootCertificate.pem'))
...
def main(argv=None):
    ...
    if num_arguments > 1:
        ...
        try:
            formats = [x.strip() for x in argv[2].split(',')]
        except IndexError:
            formats = ['best-hdr', 'best-sdr']

        _parse_aerial_files(json_folder)
        ...
        print('Usage: aerial_download.py '
              'JSON_PATH '
              '[FORMATS]\n\n'
